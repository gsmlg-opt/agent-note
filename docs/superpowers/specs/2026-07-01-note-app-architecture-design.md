...
- **Model acquisition**: BGE-M3 int8 ONNX weights are **not** fetched automatically. The plan documents manual download into `models/` in the README with `hf download gpahal/bge-m3-onnx-int8 --local-dir ./models/bge-m3-int8.onnx`. Code is written against that expected path/format.
